...
from hw_tts.model.model_config import FastSpeechConfig
...
def reprocess_tensor(batch, cut_list):
    texts = [batch[ind]["tokens"] for ind in cut_list]
    mel_targets = [batch[ind]["mel"] for ind in cut_list]
    durations = [batch[ind]["duration"] for ind in cut_list]
    og_texts = [batch[ind]["text"] for ind in cut_list]
...
def collate_fn(batch):
    len_arr = np.array([d["tokens"].size(0) for d in batch])
